...
void update_room_list(GtkWidget *view, serverRoomList *allRoom)
{
    GtkTreeModel *model;
...
void update_user_list(GtkWidget *view, TINFO *userDatabase)
{
    GtkTreeModel *model;
                        ...
                        TINFO *dataBase)
{
    ...
    GtkWidget *scrollWindow1, *scrollWindow2;
    ...
    /* Client List */
    ClientList = view_model_user(dataBase);
    gtk_container_add(GTK_CONTAINER(scrollWindow1), ClientList);
    gtk_box_pack_start(GTK_BOX(HBox), scrollWindow1, TRUE, TRUE, 0);
    gtk_widget_show(ClientList);
    gtk_widget_show(scrollWindow1);

    scrollWindow2 = gtk_scrolled_window_new(NULL, NULL);
    /* Room List */
    RoomList = view_model_room(allRoom);
    gtk_container_add(GTK_CONTAINER(scrollWindow2), RoomList);
    gtk_box_pack_start(GTK_BOX(HBox), scrollWindow2, TRUE, TRUE, 0);
...
int main(int argc, char *argv[])
{
    ...
    scrollWindow2 = gtk_scrolled_window_new(NULL, NULL);
    /* Room List */
    RoomList = view_model_room(roomList);
    gtk_container_add(GTK_CONTAINER(scrollWindow2), RoomList);
    gtk_box_pack_start(GTK_BOX(HBox), scrollWindow2, TRUE, TRUE, 0);
    gtk_widget_show(RoomList);
    gtk_widget_show(scrollWindow2);
    ...
    while (!server_Shutdown)
    {
        ...
        if (time(0) - lastDelay > 5)
        {
            update_user_list(ClientList, dataBase);
            update_room_list(RoomList, roomList);
            lastDelay = time(0);
        }
        UpdateWindow();
    }
