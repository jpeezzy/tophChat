			   ...
			   char* _Password, TINFO *userBase)
{
	...
	temp->password = malloc(sizeof(char) * 30);
	if (!temp)
	{
	...
	assert(_userName);
	assert(_name);
	if (strlen(_Password) > 30)
	{
		return NULL;
	}
	temp->userName = malloc(sizeof(char) * MAX_USER_NAME);
	memset(temp->userName, '\0', sizeof(char) * MAX_USER_NAME);
	strncpy(temp->userName, _userName, sizeof(char) * MAX_USER_NAME);
	temp->name = malloc(sizeof(char) * MAX_USER_NAME);
	memset(temp->name, '\0', sizeof(char) * MAX_USER_NAME);
	strncpy(temp->name, _name, sizeof(char) * MAX_USER_NAME);
	strcpy(temp->password, _Password);
...
int authentifyUser(char* username, char* _Password, TINFO *userbase)
{
	...
	return (strcmp(username, actualUser->userName) == 0 &&
			strcmp(_Password, actualUser->password) == 0)
			   ? 0
			   : 1;
}
...
int saveUser(TUSER *user)
{
	...
	fprintf(file, "%s|%s|%s|%d|%u|", user->userName, user->name, user->password, user->isRegistered, user->friendCount);
...
int loadUser(char* textFile, TINFO *userBase)
{
	...
	while (fgets(line, sizeof(line), file))
	{
		...
		char* _password = malloc(30 * sizeof(char));
		ui _numFriends;
		int _isRegistered;
		...
		strcpy(_name, split);
		split = strtok(NULL, "|");
		strcpy(_password, split);
		split = strtok(NULL, "|");
		_isRegistered = atoi(split);
		split = strtok(NULL, "|");
		_numFriends = (ui)atoi(split);
		TUSER *user1 = addUser(_userName, _name, _password, userBase);
		user1->isRegistered = _isRegistered;
		user1->friendCount = _numFriends;
#ifdef DEBUG
		printf("Successfully loaded one user \n");
#endif
		//printf("%s|\n", findUserByName(_userName,userBase)->userName);
	}
#ifdef DEBUG
	printf("Successfully got out of first load loop \n");
#endif
	fclose(file);
...
#ifdef DEBUG
	printf("starting to add friends\n");
#endif
...
int deleteFriend(TUSER *user, char* username)
{
	for (ui i = 0; i < user->friendCount; i++)
	{
...
int getOnlineFriends(char *username, TINFO *userbase, char* list)
{
	...
	if (temp->friendCount > 0)
	{
		if(temp->friends[0]->socket != -1)
			strcat(list, temp->friends[0]->userName);
	}
	for (ui i = 1; i < temp->friendCount; i++)
	{
		if(temp->friends[i]->socket != -1)
		{
			strcat(list, "/");
			strcat(list, temp->friends[i]->userName);
		#ifdef DEBUG
...
int main()
{
	...
	//	addUser("Justindlee","Justin", "qwer", dataBase);
...
*/
	...
	char* flist = (char*) calloc(40,sizeof(char));
	getFriends("ADMIN", dataBase, flist);
	printf("%s\n", flist);
	//	printf("authentifying user ADMIN: %d \n", authentifyUser("ADMIN", "1234", dataBase));
	//	printf("Is Admin Registered?: %d \n", findUserByName("ADMIN", dataBase)->isRegistered);
